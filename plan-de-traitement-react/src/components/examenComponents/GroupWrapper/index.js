...
const GroupWrapper = ({ componentTodisplay, examsGrouped, nbrGroupe, isModelGroup, closeModal}) => {
  const [component, setComponentName] = useState(componentTodisplay ? componentTodisplay.name : 'GROUPSUMMARY');
  ...
  return (
    <div className='wrapper'>
      {(component === 'GROUPSUMMARY') ?
        <GroupExamenSummary onAddExam={(data) => {
          onChangeComponent(data.name);}}
          examsGrouped={examsGrouped}
          nbrGroupe={nbrGroupe}
          isModelGroup={isModelGroup}
        /> : <RecapitulatifDesExamens  closeModal={closeModal}
        isModelGroup={isModelGroup} test={'test'}/>}
    </div>
  );
}
